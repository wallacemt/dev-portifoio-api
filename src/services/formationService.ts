...
import {
  formationSchema,
  formationSchemaOptional,
} from '../validations/formationValidation';
...
export class FormationService {
  ...
  public async findAllFormations(ownerId: string) {
    ...
    const formations =
      await this.formationRepository.findAllFormations(ownerId);
    const texts = {
  ...
  public async updateFormation(
    ...
    if (!formationId || formationId === ':id')
      throw new Exception('ID da formação invalida', 400);
    if (!(await this.formationRepository.findById(formationId)))
      throw new Exception('Formação não encontrado', 404);
    try {
      formationSchemaOptional.parse(formation);
      return await this.formationRepository.updateFormation(
        formation,
        formationId
      );
    } catch (e) {
