...
export class FormationController {
  ...
  private routesPrivate() {
   ...
   async getAllFormation(req: Request, res: Response) {
    ...
    try {
      const result = await this.formationService.findAllFormations(
        req.params.ownerId || ""
      );
   ...
   async create(req: Request, res: Response) {
    try {
      ...
      const FormationCreated =
        await this.formationService.addFormation(formation);
      res
        .status(201)
        .json({ message: 'Formação adicionada com sucesso', FormationCreated });
    } catch (error) {
      errorFilter(error, res);
    }
  }
   async update(req: Request, res: Response) {
    try {
      ...
      const formationUpdated = await this.formationService.updateFormation(
        formation,
        req.params.id || ""
      );
      res
